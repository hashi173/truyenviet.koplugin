const fs = require('fs');
const CryptoJS = require('./crypto-js.min.js');

const InitMangaData = {
    "decryption_key": "OENBNkU0NUYxQzREMEFDQg=="
};

const InitMangaEncryptedChapter = {
    "ciphertext": "",
    "iv": "0ec780ebd4cf7b5f6e401dd19903d681",
    "salt": ""
};

const keyStr = Buffer.from(InitMangaData.decryption_key, 'base64').toString('utf8');

// The ciphertext is provided as Base64 string, so we create a CipherParams object
var cipherParams = CryptoJS.lib.CipherParams.create({
    ciphertext: CryptoJS.enc.Base64.parse(InitMangaEncryptedChapter.ciphertext)
});

// Mode 4: Direct Key (Utf8) and IV (Hex)
try {
    var key = CryptoJS.enc.Utf8.parse(keyStr);
    var iv = CryptoJS.enc.Hex.parse(InitMangaEncryptedChapter.iv);
    var decrypted = CryptoJS.AES.decrypt(
        cipherParams,
        key,
        { iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 }
    ).toString(CryptoJS.enc.Utf8);
    
    if (decrypted) {
        console.log("Mode 4 success!");
        console.log(decrypted.substring(0, 200));
        process.exit(0);
    }
} catch (e) {
    console.log("Mode 4 failed");
}

// Mode 5: Password derivation (if salt was used, but salt in json is hex)
try {
    var salt = CryptoJS.enc.Hex.parse(InitMangaEncryptedChapter.salt);
    // Usually salt is 8 bytes, but this salt is huge!
    var derivedKey = CryptoJS.PBKDF2(keyStr, salt, { keySize: 256/32, iterations: 1000 });
    // This is getting complex, let's just output the mode 4 attempt first.
} catch (e) {}

