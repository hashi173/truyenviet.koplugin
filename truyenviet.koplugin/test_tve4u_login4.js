...
function login(token, cookies) {
    return new Promise((resolve, reject) => {
        const body = `login=${encodeURIComponent('phamthithienha17032005@gmail.com')}&matkhaune=${encodeURIComponent('Thienh@17032005')}&register=0&remember=1&cookie_check=1&_xfRedirect=${encodeURIComponent('https://tve-4u.org/')}&_xfToken=${encodeURIComponent(token)}`;
...
(async () => {
    try {
        ...
        console.log('Login Set-Cookie:', headers['set-cookie'] || 'None');
